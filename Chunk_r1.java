...
public class Chunk implements Control{
        ...
        for(int x = 0; x< chunkSize; x++){
            for(int y = 0; y< chunkSize; y++){
                for(int z = 0; z< chunkSize; z++){
                    // NEED TO USE GLOBAL POSITIONS FOR buildBlockFaces!!!
                    chunkMesh = blocks[x][y][z].buildBlockFaces(this, new Vector3f(x + chunkX, y + chunkY, z + chunkZ));
                }
            }
        }
    ...
    // The getBlock method of the chunk class needs to work with global
    //values in order to properly read neighbor chunk boundaries
    public AbstractBlock getBlock(Vector3f blocPos){
        //if((inRange(blocPos.x ) == true) && (inRange(blocPos.y ) == true) && (inRange(blocPos.z ) == true)){
        if(blocPos.x > chunkX  && blocPos.x < (chunkX + chunkSize) ){
            if(blocPos.y > chunkY   && blocPos.y < (chunkX + chunkSize)){
                if(blocPos.z > chunkZ   && blocPos.z < (chunkX + chunkSize)){
                    int x = (int) (blocPos.x - chunkX);
                    int y = (int) (blocPos.y - chunkY);
                    int z = (int) (blocPos.z - chunkZ);
                    return blocks[x][y][z];
                }
            }
        }
        ...
        if(inChunkRange(blocPos.x,blocPos.y,blocPos.z) == true){
            Vector3f v = new Vector3f(blocPos.x,blocPos.y,blocPos.z);
            return blocks[(int)v.x][(int)v.y][(int)v.z];
        }
        */
        return new AirBlock();
    }
